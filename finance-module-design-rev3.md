...
**Constraint 7:** Alerts are a Behavioral materialization of Derived signals. The unified alerts table (see Alerts System Addendum) is a stateful projection, not a source of truth. **▶ [Alerts Addendum CHANGE]** The standalone finance_alerts table described in Rev3 is superseded by the unified cross-domain alerts table. Finance alert types are domain-prefixed (e.g., finance.low_cash_runway). See Alerts System Addendum § 11.5.
...
**▶ [Alerts Addendum SUPERSESSION]** The standalone finance_alerts table and DerivedAlertCandidate schema described below are superseded by the unified cross-domain alerts system. Finance Phase 2 now targets the unified alerts table created in MVP Phase 4 — no standalone finance_alerts table is ever built. Finance alert types are domain-prefixed (e.g., low_cash_runway → finance.low_cash_runway). The DerivedAlertCandidate concept migrates to the generic AlertCandidate contract (see Alerts System Addendum § 3). The architectural principle (Derived-first + Behavioral persistence) is preserved and generalized. All finance-specific alert types, routing, and lifecycle behavior described below remain correct — only the table target and type naming change. See Alerts System Addendum § 11.5 for full supersession details.
...
**▶ [Alerts Addendum SUPERSESSION]** Finance Phase 2 now targets the unified alerts table created in MVP Phase 4 — no standalone finance_alerts table is built. Finance domain producers emit AlertCandidate objects with domain-prefixed types (finance.low_cash_runway, etc.) to the unified table.

Detection loop for rule-based alerts: finance.low_cash_runway, finance.large_transaction, finance.uncategorized_aging, finance.duplicate_import, finance.stale_pending, finance.goal_off_track, finance.unreconciled_divergence, finance.broken_transfer. Three-tier routing via unified routing engine.
...
> The Alerts Engine must deduplicate candidates via dedup_key before upserting into the unified alerts table. Duplicate signals update last_seen_at, not create new rows. **▶ [Alerts Addendum]** Now enforced by unified invariant A-04 (partial unique index WHERE status = ‘active’) and A-08 (source_hash change detection). Dedup key construction rules defined in Alerts Addendum § 4.2.

**F-15: Alert Source of Truth**

> The unified alerts table is a stateful projection of Derived signals, not a source of truth. The underlying Derived computation is authoritative. Alerts may be purged and regenerated. **▶ [Alerts Addendum]** Now enforced by unified invariant A-02 (projection-state rows are regenerable by re-running detection).
...
**10.15 Alerts Engine: Hybrid Derived + Behavioral (Rev3, updated by Alerts Addendum)**

Alerts are Derived-first (computed signals) with Behavioral persistence (stateful lifecycle management). ~The finance_alerts table is a projection, not a source of truth.~ The unified alerts table (see Alerts System Addendum) is the cross-domain projection. Finance domain producers conform to the generic AlertCandidate contract. This keeps the system compliant with the four-layer architecture while enabling stateful alert management, three-tier routing, and personalization across all domains.
...
**10.19 Alerts Engine in Phase 2, Not Phase 3 (Rev3, updated by Alerts Addendum)**

Decision surfaces are the module’s identity. Moving the Alerts Engine to Phase 2 ensures the finance module is Personal OS-native from its second iteration, not a dashboard that gets intelligence bolted on later. **▶ [Alerts Addendum]** Finance Phase 2 depends on the unified alerts table created in MVP Phase 4. No standalone finance_alerts table is ever built.
...
## Rev4           April 2026   Alerts Addendum supersession applied. Standalone finance_alerts table replaced by unified cross-domain alerts table (see Alerts System Addendum). Finance alert types domain-prefixed (e.g., finance.low_cash_runway). DerivedAlertCandidate migrated to generic AlertCandidate contract. Constraint 7 updated. F-14 and F-15 now reference unified invariants A-04/A-08 and A-02. Design decisions 10.15 and 10.19 updated. Finance Phase 2 roadmap updated to target unified alerts table from MVP Phase 4.
