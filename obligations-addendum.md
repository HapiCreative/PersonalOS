...
Behavioral   Alerts, Today Mode routing, upcoming/missed logic    Finance domain alert producers targeting unified alerts table (see Alerts System Addendum), cleanup queues
...
**4.** No match by expected date + lead_time → obligation_events row created with event_status=missed → finance.missed_obligation AlertCandidate emitted to unified alerts table.
...
**▶ [Alerts Addendum NOTE]** These alert types target the unified cross-domain alerts table (see Alerts System Addendum), not a standalone finance_alerts table. All obligation alert types use the finance domain prefix (e.g., finance.upcoming_obligation, finance.missed_obligation). They are finance domain producers conforming to the AlertCandidate contract. Entity-state alerts (upcoming, missed, spike, rate_change, expiring) use obligation_id as the primary dedup dimension. Aggregate alerts (creep, concentration, optimization) use composite dedup keys per Alerts Addendum § 4.2.
...
obligation_nodes table and node type. obligation_breakdowns table. obligation_charges_account and obligation_impacts_goal edge relations with type-pair constraints. Manual obligation creation form. Rule-based obligation alerts (finance.upcoming_obligation, finance.missed_obligation, finance.obligation_amount_spike, finance.obligation_rate_change, finance.obligation_expiring) targeting the unified alerts table from MVP Phase 4.

**▶ [Alerts Addendum NOTE]** Obligation alerts are finance domain producers conforming to the AlertCandidate contract. They target the unified alerts table, not a standalone finance_alerts table. Depends on MVP Phase 4 having created the alerts table and producer registry.

8.2 Finance Phase 3 Additions

obligation_events table (Temporal). Transaction-to-obligation matching with weighted confidence. obligation_seasonal_profiles (Derived). Seasonality detection algorithm. Rate change detection. Promoted-from-pattern workflow (two-stage lifecycle). Pattern-based alerts (finance.obligation_creep, finance.obligation_concentration, finance.obligation_optimization) classified as aggregate producers per Alerts Addendum § 4.5. Breakdown-level anomaly detection.
